 ...
 * @author olden
 */
public class WINDOWS extends outputFormatAbstract implements outputFormatInterface {

    public WINDOWS(String aggregateHex, boolean withOption249, String poolName) {
        super(aggregateHex, withOption249, poolName);
    }

    public WINDOWS(String config, String username, String password, ApplyMethod method, ArgumentParser parser) {
    ...
    public List<String> formatDhcpOptions() {
        ...
        results.add(String.format(
                "Set-DhcpServerv4OptionValue -OptionId 121 -Value 0x%s",
                aggregateHex
        ));
        if (withOption249) {
            results.add(String.format(
                    "Set-DhcpServerv4OptionValue -OptionId 249 -Value 0x%s",
                    aggregateHex
            ));
        }
